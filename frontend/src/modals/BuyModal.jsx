...
const BuyModal = ({ fetchBidsAgain, setFetchBidsAgain, energyTokensToSell, id }) => {
   ...
   const placeBid = async () => {
      try {
         ...
         const { data } = await axios.post(
            ...
            { buyerAddress, pricePerToken: updatedPricePerToken, energyTokensToSell, id },
            config,
         );
   ...
   return (
      <Dialog className="">
         ...
         <DialogContent className="font-[500] border-none px-10">
            <DialogHeader>
               <DialogTitle className="text-3xl text-[#42cdf8] mb-3  ">Buy Energy</DialogTitle>
               <span className="text-[#de1e1e]  tracking-wide pl-1">{error}</span>
               <DialogDescription className="flex flex-col pb-4 pt-1 gap-4">
                  <BarLoader color="#42cdf8" loading={loading} width="100%" height={3} />
                  <div className="flex flex-col">
                     <label className="text-[#939393] pl-1 mb-2 text-lg">Address</label>
                     <input
                        type="text"
                  ...
                  <div className="flex flex-col">
                     <label className="text-[#939393] pl-1 mb-2 text-lg">Bidding Amount</label>
                     <input
                        type="number"
                        onChange={(e) => setUpdatedPricePerToken(parseFloat(e.target.value))}
